...
class Toplevel1:
    def __init__(self, top=None):
            ...
            if not inputFile == "Please select file...." :
                if not self.OutputFolderEntry.get() =="Please select a folder....":
                    ...
                    for dataset in datasets:
                        ...
                        os.system(installationDirectory+"resources/fastuniq -i "+fileList+" -t q -o "+dedupFile1+" -p "+dedupFile2)
                        ...
                        self.logArea.configure(state='disabled')
                        self.logArea.update()
                        os.system(installationDirectory+"resources/bowtie2 -1 "+dedupFile1+" -2 "+dedupFile2+" -x "+installationDirectory+"genotyping/fastaFiles/hcmvReference -S alignmenthsbfy43223.sam >null 2>&1")
                        self.logArea.configure(state='normal')
                        self.logArea.insert(tk.END, "Done!\n")
                        self.logArea.see(tk.END)
                        self.logArea.configure(state='disabled')
                        ...
                        self.logArea.configure(state='disabled')
                        self.logArea.update()
                        os.system(installationDirectory+"resources/samtools view -bS -h alignmenthsbfy43223.sam > alignmenthsbfy43223.bam")
                        self.logArea.configure(state='normal')
                        self.logArea.insert(tk.END, "Done!\n")
                        self.logArea.see(tk.END)
                        self.logArea.configure(state='disabled')
                        ...
                        self.logArea.configure(state='disabled')
                        self.logArea.update()
                        os.system(installationDirectory+"resources/samtools sort -o alignmenthsbfy43223_sorted.bam alignmenthsbfy43223.bam")
                        self.logArea.configure(state='normal')
                        self.logArea.insert(tk.END, "Done!\n")
                        self.logArea.see(tk.END)
                        self.logArea.configure(state='disabled')
                        ...
                        self.logArea.configure(state='disabled')
                        self.logArea.update()
                        os.system(installationDirectory+"resources/samtools depth  alignmenthsbfy43223_sorted.bam  |  awk '{sum+=$3} END { print sum/NR}' >avCoverage.txt")
                        self.logArea.configure(state='normal')
                        self.logArea.insert(tk.END, "Done!\n")
                        self.logArea.see(tk.END)
                        self.logArea.configure(state='disabled')
                        ...
                        self.logArea.configure(state='disabled')
                        self.logArea.update()
                        os.system(installationDirectory+"resources/jellyfish-2.2.10/bin/jellyfish count -m 17 -s 100M -t 8 -C "+dedupFile1+" -o "+dedupFile1+"_kmerCount.jf")
                        ...
                        os.system(installationDirectory+"resources/jellyfish-2.2.10/bin/jellyfish count -m 17 -s 100M -t 8 -C "+dedupFile2+" -o "+dedupFile2+"_kmerCount.jf")
                        ...
                        os.system(installationDirectory+"resources/jellyfish-2.2.10/bin/jellyfish merge "+dedupFile1+"_kmerCount.jf "+dedupFile2+"_kmerCount.jf")
                        self.logArea.configure(state='normal')
                        self.logArea.insert(tk.END, "Done!\n")
                        self.logArea.see(tk.END)
                        self.logArea.configure(state='disabled')
        ...
        def plotGenotypes():
            ...
            if not self.OutputFolderEntry.get() == "Please select a folder....":
                ...
                for name in sampleList:
                    pdfOutName += name+"_"
                pdfOutName += ".pdf"
                existing_pdf = PdfFileReader(file("plot2.pdf", "rb"))
                new_pdf =  PdfFileReader(file(installationDirectory+"genotyping/legend2.pdf", "rb"))
                output = PdfFileWriter()
                page = existing_pdf.getPage(0)
                page.mergePage(new_pdf.getPage(0))
                output.addPage(page)
                outputStream = file(pdfOutName, "wb")
                output.write(outputStream)
                outputStream.close()
                os.system("rm -f plot.pdf plot2.pdf legend.pdf")
                os.system("mv "+pdfOutName+" "+self.OutputFolderEntry.get())
        ...
        self.OutputFileLabel = tk.Label(top)
        ...
        self.OutputFileButton = tk.Button(top,command=openOutputFolder)
        ...
        self.CutoffLabel.place(x=20, y=180,height=20, width=120)
        ...
        self.logFrame.place(x=20, y=300, height=180, width=800)
